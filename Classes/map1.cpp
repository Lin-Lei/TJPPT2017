...
bool Map1::init() {
	...
	//创建双人对战地图
	map1 = TMXTiledMap::create("map/map1/map1.tmx");
	map1->setAnchorPoint(Vec2(0.5, 0.5));
	map1->setPosition(Vec2(origin.x + visibleSize.width / 2 - 80, origin.y + visibleSize.height / 2));
	addChild(map1, 1);

	building = map1->getLayer("building");
	barrierLayer = map1->getLayer("barrier");

	//返回图片菜单
	auto returnItem = MenuItemImage::create(
		"button/backnormal.jpg",
		"button/backselect.jpg",
		CC_CALLBACK_1(Map1::menuReturnCallback, this));
	...
	hero->setScene(building, barrierLayer, map1);
	bubble->setScene(building, barrierLayer, map1);
	hero->setPosition(Vec2(160, 50));
	this->addChild(hero, 10, HERO_1);

	hero2 = Hero::create("hero2Down.png", 2);
	hero2->setScene(building, barrierLayer, map1);
...
//进入游戏场景
void Map1::onEnterTransitionDidFinish() {
...
//返回主界面
void Map1::menuReturnCallback(cocos2d::Ref* pSender) {
	if (soundSet) SimpleAudioEngine::getInstance()->playEffect("music/clip.mp3");
	Director::getInstance()->popScene();
}

void Map1::update(float dt)//每秒60次更新
{
	if (hero->die) {//?????
		hero2->win();
		hero->die = false;
...
bool Map1::checkArrow(EventKeyboard::KeyCode keyCode)//检测P1按键
{
...
bool Map1::checkP2(EventKeyboard::KeyCode keyCode)//检测P2按键
{
